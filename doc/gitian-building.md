...
*Setup instructions for a gitian build of 5GL using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the 5GL
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-5gl-host/README.md).
...
Clone the git repositories for 5gl and gitian and then checkout the 5gl version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/5gl/5gl
cd 5gl
...
Follow the instructions in [doc/release-process.md](release-process.md) in the 5gl repository
...
To build 5GL (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the 5gl repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/5gl/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/5gl/5gl
...
For example:
```bash
URL=https://github.com/laanwj/5gl.git
COMMIT=2019_03_windows_unicode_path
./bin/gbuild --commit 5gl=${COMMIT} --url 5gl=${URL} ../5gl/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit 5gl=${COMMIT} --url 5gl=${URL} ../5gl/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit 5gl=${COMMIT} --url 5gl=${URL} ../5gl/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/5gl-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/5gl-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/5gl-build.assert
```
...
[5gl/gitian.sigs](https://github.com/5gl/gitian.sigs/) repository, or if that's not possible create a pull
