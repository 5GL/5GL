...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete 5GL database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no 5GL datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old 5GL datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected 5GL 0.7 datadir."
    ;;
  3)
    echo "Detected 5GL pre-0.8 datadir."
    ;;
  4)
    echo "Detected 5GL 0.8 datadir."
    ;;
esac
